...
function History({ events, people, onEdit, onDelete, babyId, onImported }: { events: BabyEvent[]; people: Record<string, string>; onEdit: (event: BabyEvent) => void; onDelete: (event: BabyEvent) => void; babyId?: string; onImported: (message: string) => void }) {
  ...
  return <section><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-2xl font-black">Event history</h2><label className="tap flex cursor-pointer items-center gap-2 rounded-2xl bg-[#4f7b68] px-5 font-black text-white"><FileUp />Import CSV<input type="file" accept=".csv,text/csv" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) importFile(file); }} /></label></div>
